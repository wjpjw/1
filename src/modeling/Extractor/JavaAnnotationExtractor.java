...
//			}
			//当遇到"/*"的处理
				...
				if(inAnotation == false){
					matchStart = i+2;
					res.add(new RecognizedString(str.substring(otherStart, i),otherStart, i,CodeType.MAIN));
					inAnotation = true;
				}
			}
			//当遇到"*/"的处理
				...
				if(inAnotation = true){
					inAnotation = false;
					otherStart = i+2;
					res.add(new RecognizedString(str.substring(matchStart, i),matchStart, i,CodeType.ANOTATION));
				}
			}
		}
		
		if(!inAnotation&&otherStart!=l)
			res.add(new RecognizedString(str.substring(otherStart, l),otherStart, l,CodeType.MAIN));
		if(inAnotation&&matchStart!=l)
			res.add(new RecognizedString(str.substring(matchStart, l),matchStart, l,CodeType.ANOTATION));
